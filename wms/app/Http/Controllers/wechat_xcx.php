...
class wechat_xcx extends Controller
{
    ...
    function index(){
        if (isset($_GET["code"])) {
            $log_data = json_decode(file_get_contents("https://api.weixin.qq.com/sns/jscode2session?appid=wxb8f4b895aa4f7ad9&secret=1776edd314e09b2c9c9722a5fd771f1e&js_code=".$_GET["code"]."&grant_type=authorization_code"));
            if (isset($log_data->errcode)) {
                echo $log_data;
            } else if (isset($log_data->openid)) {
                $users = \App\User::where("code",$log_data->openid)->get();
                $info = json_decode($_GET["info"]);
                if (sizeof($users) == 1) {
                    $user = $users[0];
                    Auth::loginUsingId($user->id,true);
                    $user->session_key = $log_data->session_key;
                    ...
                    $new_user = \App\User::create([
                        ...
                        'mobile' => '',
                        'avatar' => '',
                        'session_key' => $log_data->session_key
                    ]);
