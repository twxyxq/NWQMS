...
@define $sdata = array(array("title" => "焊口数", "name" => "amount", "type" => "bar"));

@push('scripts')
<script type="text/javascript">
	function generate_title(){
		return ($("[name='ild']").val()==-1?"":($("[name='ild']").val()+"号机组"))+($("[name='sys']").val()==-1?"":$("[name='sys']").val())+"完成焊口"+$("[name='period']").find("option:selected").text();
	}
</script>
@endpush