...
@define $sdata = array(array("title" => "检验数", "name" => "amount", "type" => "bar"),array("title" => "报告数", "name" => "report", "type" => "bar"));

@push('scripts')
<script type="text/javascript">
    function generate_title(){
        return ($("[name='ild']").val()==-1?"":($("[name='ild']").val()+"号机组"))+($("[name='sys']").val()==""?"":$("[name='sys']").val())+($("[name='emethod']").val()==""?"":$("[name='emethod']").val())+"检验数量"+$("[name='period']").find("option:selected").text();
    }
</script>
@endpush