...
@section('content')
    ...
    @define $wjs = \App\wj::select(array(DB::raw("*"),DB::raw(SQL_VCODE." as wj_code"),DB::raw(SQL_EXAM_RATE." as rate"),DB::raw(SQL_BASE_TYPE." as type")))->whereIn("id",multiple_to_array($id))->withoutGlobalScopes()->get();
    @foreach($wjs as $wj)
        ...
        @define $deleted = \Carbon\Carbon::parse($wj->deleted_at)->lt(\Carbon\Carbon::parse('2037-12-31'))

        <div class="container" {{$deleted?'title=已删除 style=background-color:lightgray;opacity:0.8':''}}>
            <div>
                <ul id="tskTabs" class="nav nav-tabs" role="tablist">
                    <li role="presentation" class="active"><a href="#info" id="info-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="false">焊口信息{!!$deleted?'（<span style="color:red">已删除</span>）':''!!}</a></li>
                    ...
                    <li role="presentation" class=""><a href="#history" role="tab" id="history-tab" data-toggle="tab" aria-controls="profile" aria-expanded="true">历史记录</a></li>
                </ul>
                <div id="tskTabContent" class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane fade" id="history" aria-labelledby="history-tab">
                        @define $histories = \App\modify_history::where("model","App\wj")->where("model_id",$wj->id)->get()
                        @foreach($histories as $history)
                            @define $user = \App\user::find($history->created_by)
                            <div class="col-sm-12" style="padding: 10px">
                                修改日期：{{$history->created_at}}<br>
                                修改内容：<br>
                                @define $history_content = json_decode($history->history,true)
                                @foreach($history_content as $c)
                                    @if($c["key"] == "deleted_at")
                                        @if(is_array($c["old"])?$c["old"]["date"]:$c["old"] == '2037-12-31 00:00:00')
                                            【删除恢复】
                                        @elseif(is_array($c["new"])?$c["new"]["date"]:$c["new"] == '2037-12-31 00:00:00')
                                            【删除数据】
                                        @else
                                            {{$c["key"]}} ： <del>{{is_array($c["old"])?$c["old"]["date"]:$c["old"]}}</del> -> {{is_array($c["new"])?$c["new"]["date"]:$c["new"]}}<br>
                                        @endif
                                        <br>
                                    @elseif($c["key"] != "updated_at")
                                        {{$c["key"]}} ： <del>{{is_array($c["old"])?$c["old"]["date"]:$c["old"]}}</del> -> {{is_array($c["new"])?$c["new"]["date"]:$c["new"]}}<br>
                                    @endif
                                @endforeach
                                修改人：{{$user->name}}
