<div id="1" class="welding_record" style="display:inline-block">
<style>
...
<table border="1" width="675" id="tb01" style="font-size:13px;text-align:center;border-collapse:collapse;overflow=hidden;TABLE-LAYOUT:fixed;word-break:break-all; word-wrap:break-all;">
	...
	<tr>
		...
		<td colspan="7" height="32" align="center" valign="bottom" style="border-left:solid 1px;border-bottom:solid 0px;border-right:solid 1px"><font size="5"><strong>阳江核电站{{$info->wj_type}}{{$tsk->tsk_ft}}焊接控制单</strong></font></td>
		<td rowspan="2" colspan="1"><img src="/images/qrcode/{{10000000000+PJCODE*1000000+$info->id}}.png" /></td>
	</tr>
	<tr>
		<td colspan="7" height="24" align="right" style="border-left:solid 1px;border-top:solid 0px;border-bottom:solid 1px;border-right:solid 1px">{{10000000000+PJCODE*1000000+$info->id}}</td>
	</tr>
	<tr align="left">
	...
	<tr>
		...
		<td colspan="2" height="27">#(IF(q_pipe_type='仪表','仪表及管道','管道内'))#清洁度</td>
		<td colspan="2">#(IF(q_pipe_type='仪表','仪表与管道对口','插套间隙'))#</td>
		<td colspan="2">#(IF(q_pipe_type='仪表','仪表插入深度','管道平直度'))#</td>
		<td>#(IF(q_pipe_type='仪表','结论','错口值'))#</td>
		<td>#(IF(q_pipe_type='仪表','备注','结论'))#</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="10" height="#(IF(q_pipe_type='油系统',50,38))#" align="left">
		#(IF(q_pipe_type='油系统','质量要求：1、对接管平直度：DN＜100mm:≤1mm/200mm；DN≥100mm:≤2mm/200mm。2、组对错口值：不宜超过管道壁厚的10％，且≤1mm。3、承插焊接头，承口与插口的轴向不宜留有间隙。4、对口前应用丙酮（或无水乙醇）和白绸布对油管道内部进行擦拭清理，然后用白绸布擦拭，直至白绸布无污迹。5、管口封堵良好',IF(q_pipe_type='仪表','质量要求：1、仪表与被测金属表面光滑无毛刺。2、仪表与管道对口安装凡是符合设计要求。3、仪表插入深度符合设计要求。','质量要求：1、对接管平直度：当公称直径：DN＜100mm时，应≤1mm/200mm；当公称直径：DN≥100mm时，应≤2mm/200mm。2、组对错口值：不宜超过管道壁厚的10％，且≤1mm')))#
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2" height="60">RT {{$info->RT}} UT {{$info->UT}}<br>PT {{$info->PT}} MT {{$info->MT}}</td>
		<td>#(wps_wpq_code)#</td>
		<td>{{$wps->wps_code}}</td>
